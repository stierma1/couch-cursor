...
function request(objOrUri, returnFunc){
  ...
  try{
    //console.log(path.join(__dirname, "../__mockData__/", method +"-" + pieces[pieces.length - 2] + "-" + pieces[pieces.length - 1]))
    var data = fs.readFileSync(path.join(__dirname, "../__mockData__/", method +"-" + pieces[pieces.length - 2] + "-" + pieces[pieces.length - 1]), "utf8");
  } catch(e){
    //console.log(e)
